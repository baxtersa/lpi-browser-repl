...
Elm.Main.make = function (_elm) {
   ...
   $Signal = Elm.Signal.make(_elm);
   var _op = {};
   var clearMailbox = $Signal.mailbox({ctor: "_Tuple0"});
   var clear = Elm.Native.Port.make(_elm).outboundSignal("clear",function (v) {    return [];},clearMailbox.signal);
   var compileMailbox = $Signal.mailbox({ctor: "_Tuple0"});
   var compile = Elm.Native.Port.make(_elm).outboundSignal("compile",function (v) {    return [];},compileMailbox.signal);
   var update = F2(function (action,model) {    return model;});
   var button = F4(function ($class,title,prompt,address) {
      return A2($Html.div,
      _U.list([$Html$Attributes.$class($class),$Html$Attributes.title(title),A2($Html$Events.onClick,address,{ctor: "_Tuple0"})]),
      _U.list([$Html.text(prompt)]));
   });
   var view = F2(function (address,model) {
      return A2($Html.div,
      _U.list([$Html$Attributes.$class("options")]),
      _U.list([A4(button,"button","Type-check and evaluate the code (Cmd+Enter)","Compile",compileMailbox.address)
              ,A4(button,"button","Clear code from the editor","Clear",clearMailbox.address)]));
   });
   var initialModel = {};
   var Model = {};
   var NoOp = {ctor: "NoOp"};
